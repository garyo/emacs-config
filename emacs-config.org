...
To create a source block (matched pair), use ~C-c C-,~.
...
Logseq PKM system keeps its files as Markdown, so they can be edited by Emacs as well.
Here's a simple integration: a transient menu for going to today's journal entry.
...
;;; M-j is a good binding; normally it's bound to default-indent-new-line which
;;; I don't use, and anyway that's still on C-M-j if needed.
(use-package avy
  :ensure nil
